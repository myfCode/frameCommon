...
(function(window, document, Math) {
    ...
    var utils = (function() {
        ...
        */
        ...
        me.click = function(e) {
            ...
            if (!(/(SELECT|TEXTAREA)/i).test(target.tagName)) {//lewis |INPUT
    ...
    function IScroll(el, options) {
        ...
        this.options = {
            ...
            click: true, //自定义事件，配置为 string,需要执行的自定义事件
        ...
        this.options.resizePolling = this.options.resizePolling === undefined ? 60 : this.options.resizePolling; //默认值 60ms，控制下拉刷新时间
    ...
    IScroll.prototype = {
        ...
        refresh: function() {
            ...
            function superRefresh(_this) {
                if (!(window.screen.availHeight > 1)) {
                    setTimeout(superRefresh, 1000 / 60);
                } else {
                    init(_this);
                }
            }
            ...
            function init(_this) {
                var rf = _this.wrapper.offsetHeight; // Force reflow
                ...
                var limit;
                if (_this.options.infiniteElements) {
                    _this.options.infiniteLimit = _this.options.infiniteLimit || Math.floor(2147483645 / _this.infiniteElementHeight);
                    limit = -_this.options.infiniteLimit * _this.infiniteElementHeight + _this.wrapperHeight;
                }
                _this.maxScrollY = limit !== undefined ? limit : _this.wrapperHeight - _this.scrollerHeight;

                /* REPLACE END: refresh */

                _this.hasHorizontalScroll = _this.options.scrollX && _this.maxScrollX < 0;
                _this.hasVerticalScroll = _this.options.scrollY && _this.maxScrollY < 0;
                ...
                _this.wrapperOffset = utils.offset(_this.wrapper);

                _this._execEvent('refresh');
